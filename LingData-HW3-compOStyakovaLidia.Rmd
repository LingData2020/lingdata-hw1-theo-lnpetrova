...
a. According to the histogram, we can see that number of elements is equal to 20 in this case(after adding all the frequencies 1+10+4+4+1). There are 5 elements in the sample that exceed 5 (4 between 5 and 10 and 1 between 15 and 20). So, the proportion is 5/20 = 1/4
b. The median should be between 0 and 5 because in this case there will be an equal amount of values to the left and to the right from the median.
c. Firstly, an element $7$ will become a median. Secondly, the frequency of the column with values between 0 and 5 will increase. 
...
The first sample.
The first sapmle has a larger distribution of values - approximately between 375 and 625 relatively the average value, while the second sample has a distribution of values between 440 and 580. It means that the first
```{r, echo=FALSE, fig.height=8} sample has a larger variance.
set.seed(12)
...
Which of the samples has a larger variance? Explain your answer.
The first sample.
As we can see, the first sample has a larger distribution of values - between -10 and 10 while the value distribution of the second sample is only between 8 and 12. It means that the first sample has a larger variance. 
...
```
No, it's false. Most of the histogram area lies before 35 that means less than 50 % students made more than 35 mistakes.
```
...
```
No, it's false. Most of the histogram area is after 10 that means most students made more that 10 mistakes.
```
Explain your answer below:
...
```
Explain your answer below:
The histogram area is shifted to the left that means the median of mistakes should be shifted to the left, as well, and it's not 40 as expected. It is noticable that there are no so many bins between 60 and 80 and we can consider the area before 60. Obviosly, the mediana is not equal to 10, only 20 and 30 left. If we choose 20, we can see that the histogram area before 20 is smaller than the historam area after 20. So, it means that the median of mistakes should be closer to 30.
...
```
As we can see, p = 1/ 1 000 000 which is less than the standart value 0,05 and the null hypothesis could be rejected. However, it's clear that these sample data are consistent with the null hypothesis in most of the cases and there is no need to reject that no success is possible.
```
Explain your answer below:
...
```{r}
hist(age, col= "light blue", breaks=5)
```
...
```
H0: The probability for verbs in verses written in 1920s in the rhyming positions is the same as for verbs in general
H1: The probability for verbs in verses written in 1920s in rhyming positions differs from the probability for verbs in general
```
...
```{r}
poetry %>% filter(poetry$Decade=='1920s') -> poetry_20s
poetry_20s %>% filter(poetry_20s$UPoS=='VERB') -> verbs_20s
```
...
```{r}
binom.test(nrow(verbs_20s), nrow(poetry_20s), p=0.17)
```
...
```
p-value = 0.2297 
p-value > 0,05 that means we can't reject the null hypothesis. So, no, verbs are not used in the rhyming position more often or less often than expected.
```
...
```{r}
poetry %>% filter(poetry$Decade=='1820s') -> poetry_1820s
poetry_1820s %>% filter(poetry_1820s$UPoS=='VERB') -> verbs_1820s
binom.test(nrow(verbs_1820s), nrow(poetry_1820s), p=0.17)
```

Write down your general conlusions about data provided for both 1920s and 1820s data.
```
p-value = 0.8873 
p-value > 0,05 that means we can't reject the null hypothesis. Verbs are not used in the rhyming position n verses written in 1820s more often or less often than expected neither. So, we can conclude that probabilities for verbs in general and in a particular decade are more or less equal. 
```
...
```
H1: The population mean of vowel duration in speaker 'shg05' is not equal to 73 ms
...
```{r}
icedata <- read_csv("https://raw.githubusercontent.com/LingData2019/LingData2020/master/data/icelandic.csv")
icedata %>% filter(icedata$speaker == 'shg05') -> shg05
shg05
...
```
p-value = 0.05172
p-value > 0,05 that means that we can't reject the null hypothesis and the population mean of vowel duration in speaker 'shg05' can be equaled to 73 ms. Actually, it's important to notice that p-value is really close to 0,05 and there is a possibility that the null hypothesis can be rejected. 
```
